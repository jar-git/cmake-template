...
{
  auto socket{make_stream_socket(socket_family::domain)};
...
TEST_F(stream_socket_test, connect)
{
  ...
  auto closing_connection = async_accept([](auto client_socket) {
...
TEST_F(stream_socket_test, connect_refused)
{
  ...
  EXPECT_THROW(
      {
        try {
          auto no_connection{make_domain_address(NO_ADDRESS)};
...
TEST_F(stream_socket_test, send)
{
  ...
  auto closing = async_accept([connect = std::move(connect)](auto client_socket) mutable {
    EXPECT_TRUE(client_socket->is_open());
...
TEST_F(stream_socket_test, shutdown_send)
{
  ...
  auto closing = async_accept([connect = std::move(connect), shutdown](auto client_socket) mutable {
    EXPECT_TRUE(client_socket->is_open());
