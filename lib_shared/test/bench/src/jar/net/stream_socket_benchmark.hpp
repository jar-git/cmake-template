...
///
#ifndef JAR_NET2_STREAM_SOCKET_BENCHMARK_HPP
#define JAR_NET2_STREAM_SOCKET_BENCHMARK_HPP
...
public:
  ...
  basic_stream_socket& client() noexcept { return *m_client_socket; }
...
private:
  ...
  void start_server()
  {
    ...
    m_server_thread = std::thread{[this]() {
      m_server_socket->accept([this](auto client_socket) {
  ...
  std::unique_ptr<basic_stream_socket> m_server_socket{make_stream_socket(socket_family::domain)};
  std::unique_ptr<basic_stream_socket> m_client_socket{make_stream_socket(socket_family::domain)};
  std::unique_ptr<socket_address> m_server_address{make_domain_address(SOCKET_ADDRESS)};
  std::thread m_server_thread;
};

}  // namespace jar::net::bench

#endif  // JAR_NET2_STREAM_SOCKET_BENCHMARK_HPP
