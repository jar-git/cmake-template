...
void socket::bind(const jar::net::socket_address& local_address)
{
  contract::no_system_error(::unlink(&local_address.address()->sa_data[0]));
  ::socklen_t length{static_cast<::socklen_t>(local_address.size())};
  contract::no_system_error(::bind(native(), local_address.address(), length));
}
...
void socket::shutdown(jar::net::shutdown_mode mode)
{
